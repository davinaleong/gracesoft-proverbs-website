...
- No custom backend — the `mailto:` CTAs stay as the primary contact path. **Updated:** Support now also embeds a third-party contact form (Capture, at `capture.gracesoft.dev`) as an `<iframe>` on `support.astro`. It's a client-side embed of a hosted form, not a backend the site itself runs, so this doesn't add a backend of our own — but it does mean Privacy needs a line noting Capture as a data processor for form submissions (tracked as an open item in `05-implementation-log.md`).